...
export default function (props: IProps) {
  ...
  useEffect(() => {
    if (id) {
      ...
      const label = cell.getLabelAt(0)?.attrs?.label.text
      setAttr('label', label)
    }
  }, [id])
  ...
  const onLabelChange = (e: React.FocusEvent<HTMLInputElement>) => {
    const val = e.target.value
    setAttr('label', val)
    cellRef.current!.setLabelAt(0, val)
  }
    ...
    <Tabs defaultActiveKey="1">
      <TabPane tab="线条" key="1">
        <Row align="middle">
          <Col span={8}>文本</Col>
          <Col span={14}>
            <Input type="text" value={attrs.label} onChange={onLabelChange} />
          </Col>
        </Row>
        <Row align="middle">
